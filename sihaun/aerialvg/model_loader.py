...
AERIALVG_ROOT = Path(__file__).resolve().parent
for path in (AERIALVG_ROOT, REPO_ROOT):
    if str(path) not in sys.path:
        sys.path.insert(0, str(path))

from .model import build_model

DEFAULT_CONFIG_FILE = AERIALVG_ROOT / "config" / "default.py"
...
def resolve_repo_path(path_like: str | Path | None) -> Path | None:
    ...
    search_roots = [Path.cwd(), REPO_ROOT, AERIALVG_ROOT]
...
def load_aerialvg_config(config_file: str | Path | None = None):
...
def build_aerialvg_model(
    config_file: str | Path | None = None,
    checkpoint_path: str | Path | None = None,
    device: str = "cpu",
    strict: bool = False,
):
    cfg = load_aerialvg_config(config_file)
    cfg.device = device
