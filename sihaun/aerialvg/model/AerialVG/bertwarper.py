...
class BertModelWarper(nn.Module):
    def __init__(self, bert_model):
        ...
        self._get_head_mask_fn = getattr(bert_model, "get_head_mask", None)
        self.get_extended_attention_mask = bert_model.get_extended_attention_mask
        self.invert_attention_mask = bert_model.invert_attention_mask
        self._extended_attention_mask_params = set(
            inspect.signature(bert_model.get_extended_attention_mask).parameters.keys()
        )

    def _get_head_mask(self, head_mask, num_hidden_layers):
        if self._get_head_mask_fn is not None:
            return self._get_head_mask_fn(head_mask, num_hidden_layers)
        ...
        if head_mask.dim() == 1:
            ...
            raise ValueError(
                "head_mask must have dim 1 or 2, but got {}".format(head_mask.dim())
            )
    ...
    def _build_extended_attention_mask(self, attention_mask, input_shape, device):
        ...
        if "dtype" in self._extended_attention_mask_params:
            kwargs["dtype"] = self.embeddings.word_embeddings.weight.dtype
        if "device" in self._extended_attention_mask_params:
            kwargs["device"] = device
        return self.get_extended_attention_mask(attention_mask, input_shape, **kwargs)
...
def generate_masks_with_special_tokens_and_transfer_map(tokenized, special_tokens_list, tokenizer):
    ...
    cate_to_token_mask_list = [
        ...
        else torch.zeros((0, num_token), device=input_ids.device, dtype=torch.bool)
