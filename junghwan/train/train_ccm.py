#!/usr/bin/env python3
"""
Training script for Grounding DINO on AerialVG dataset.
...
Usage (single GPU):
    python train.py \
        ...
        --output_dir outputs/aerialvg_swinb
...
# Ensure the script works when run from vision_code/
...
def parse_args():
    parser = argparse.ArgumentParser("Grounding DINO Training on AerialVG")

    # Model
    parser.add_argument(
        "--config_file",
        required=True,
        help="Path to model config (.py), e.g. groundingdino/config/GroundingDINO_SwinB_cfg.py",
    )
    parser.add_argument(
        "--pretrained_weights",
        default=None,
    ...
    parser.add_argument(
        "--data_dir",
        default="/data2/huggingface/AerialVG",
        help="Root directory of AerialVG dataset (HuggingFace format)",
    )
    ...
    # Optimizer
    parser.add_argument("--lr", type=float, default=1e-4, help="LR for transformer/heads")
    parser.add_argument("--lr_backbone", type=float, default=1e-5, help="LR for Swin backbone")
    parser.add_argument("--lr_bert", type=float, default=1e-5, help="LR for BERT encoder")
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=None,
        help="Epoch milestones for MultiStepLR (×0.1). None → cosine annealing.",
    )
...
def build_scheduler(optimizer, args, steps_per_epoch: int):
    ...
    if args.lr_drop is not None:
        # Multi-step: step per epoch
        return torch.optim.lr_scheduler.MultiStepLR(
            optimizer, milestones=args.lr_drop, gamma=0.1
        ), False  # (scheduler, is_per_iter)

    # Cosine with warmup: step per iteration
    base_lr = args.lr
    ...
    def lr_lambda(step):
        ...
        return max(1e-7 / max(base_lr, 1e-10), cosine)
...
def load_pretrained(model: nn.Module, path: str):
    ...
    if is_main_process():
        print(f"[Pretrained] Missing keys: {len(missing)}  Unexpected keys: {len(unexpected)}")
    return model
...
def main():
    ...
    weight_dict["loss_ccm"] = 1.0  # CCM density loss (not duplicated in aux)
    ...
    train_split_key = "train" if "train" in raw_ds else list(raw_ds.keys())[0]

    train_dataset = AerialVGDataset(
        hf_split=raw_ds[train_split_key],
        image_dir=image_dir,
        tokenizer=tokenizer,
    ...
    scheduler, scheduler_per_iter = build_scheduler(
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        # Step multi-step scheduler per epoch (cosine is stepped per iter inside loop)
        ...
        if is_main_process():
            ...
            loss_total = train_stats.get("loss_total", float("nan"))
            lr_cur = train_stats.get("lr", float("nan"))
            print(
                f"Epoch {epoch:3d} | loss={loss_total:.4f} | lr={lr_cur:.2e}"
            )
