...
class GroundingDINO(nn.Module):
    ...
    def forward(self, samples: NestedTensor, targets: List = None, **kw):
        ...
        hs, reference, hs_enc, ref_enc, init_box_proposal, counting_output = \
            self.transformer(
        ...
        if counting_output is not None:
            out["pred_bbox_number"] = counting_output  # [bs, 4] — CCM density class logits

        # for intermediate outputs
        if self.aux_loss:
            out['aux_outputs'] = self._set_aux_loss(outputs_class, outputs_coord_list)

        # GPQ: record per-slot score using final layer outputs.
        # Passes pred_boxes + targets so transformer can compute IoU-based score
        # if `gpq_score_method == "iou"` (Tier 2 default). Falls back to confidence
        # when set to "confidence" or when targets/pred_boxes are absent.
        if self.training and getattr(self.transformer, 'use_gpq', False):
            self.transformer.record_query_scores(
                outputs_class[-1],
                pred_boxes=outputs_coord_list[-1],
                targets=targets,
            )
