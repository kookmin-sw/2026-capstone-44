...
- Loss: CE over top-K candidates (positive = highest IoU with GT)
"""
...
TOPK          = 15
IOU_THRESHOLD = 0.1
SRBM_LAYERS   = 2    # ablation: try 3 for paper
    ...
    for epoch in range(EPOCHS):
        ...
        for batch_idx, (images, captions, gt_boxes) in enumerate(
            ...
            try:
                ...
                for i in range(bs):
                    attempted += 1
                    ious = box_iou_batch(topk_boxes_xyxy[i], gt[i].unsqueeze(0)).squeeze(-1)
                ...
                loss = nn.CrossEntropyLoss()(srbm_scores, labels)
