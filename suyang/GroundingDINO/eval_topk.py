"""
Top-1 / Top-5 evaluation with SRBM.
Usage:
  python eval_topk.py --ckpt output/srbm/best.pth
"""
...
DEVICE   = "cuda:0"
TOPK     = 15  # default, can be overridden by --topk
...
def evaluate(model, loader):
    ...
    for images, captions, gt_boxes in tqdm(loader, desc="Eval", leave=False):
        try:
            ...
            srbm_logits = srbm(topk_features, topk_coords, text_dict)
            srbm_scores = srbm_logits.max(dim=-1).values[0]  # [K]

            ranked_idx  = srbm_scores.argsort(descending=True)
            ranked_boxes = cxcywh_to_xyxy(topk_coords[0])[ranked_idx]

            # Top-1
            iou1 = box_iou(ranked_boxes[0].tolist(), gt.tolist())
            top1_correct += float(iou1 > 0.5)

            # Top-5
            hit5 = any(box_iou(ranked_boxes[i].tolist(), gt.tolist()) > 0.5
                       for i in range(min(5, len(ranked_boxes))))
...
def main():
    ...
    parser.add_argument("--topk",       type=int, default=TOPK,
                        help="number of candidates for SRBM (must match training)")
    parser.add_argument("--anno",       type=str, default=VAL_ANNO,
                        help="annotation jsonl path (val or test)")
    ...
    print(f"Loading SRBM ckpt: {args.ckpt}")
    ...
    ds     = AerialVGEvalDataset(args.anno, IMG_ROOT)
