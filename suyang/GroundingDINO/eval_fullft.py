"""
Evaluate fully fine-tuned GDINO (checkpoint_baseline.pth) on AerialVG val.
No LoRA, no SRBM — just full FT GDINO.
"""
...
def main():
    ...
    print(f"Loading vanilla GDINO config + full-FT weights...")
    model = load_model(CONFIG, BACKBONE)  # build with vanilla weights first
    # Override with full-FT weights
    ...
    if "epoch" in ckpt: print(f"  ckpt epoch: {ckpt['epoch']}")
    if "val_loss" in ckpt: print(f"  ckpt val_loss: {ckpt['val_loss']}")
    ...
    print(f"\n{'='*50}")
    print(f"  Full FT GDINO (checkpoint_baseline.pth)")
