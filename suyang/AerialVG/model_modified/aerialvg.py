...
from .srbm import SpatialRelationBiasModule  # added: our SRBM
...
class AerialVG(nn.Module):
    ...
    def forward(self, samples: NestedTensor, targets: List = None, **kw):
            ...
            # [H1] top-k 후보의 예측 박스 좌표 추출 (cx, cy, w, h) normalized
            topk_coords = torch.gather(outputs_coord_list[-1], 1, topk_indices.unsqueeze(-1).expand(-1, -1, 4))
...
def build_aerialvg(args):
    ...
    # === SRBM substitution (our module replaces CrossSelfRelationTransformer) ===
    if getattr(args, "use_srbm", False):
        relation_transformer = SpatialRelationBiasModule(
            d_model=256, num_heads=8,
            num_layers=getattr(args, "srbm_layers", 3),
