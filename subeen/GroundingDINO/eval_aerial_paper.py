"""
AerialVG 논문 GitHub 버전 eval — baseline 전용
모델 구조: groundingdino_original.py + transoferm_original.py
"""
import argparse
import json
import os
...
from groundingdino.models.GroundingDINO.groundingdino_original import GroundingDINO
from groundingdino.models.GroundingDINO.transoferm_original import build_transformer
...
# (eval_aerial_modified.py와 동일 정의 — split 일관성 보장)
# --------------------------------------------------------------------------- #
...
def eval_vg(model, data_loader_val, topk, device):
    ...
    for samples, targets in tqdm(data_loader_val, desc="Evaluating", unit="batch"):
        ...
        logits = outputs["pred_logits"].sigmoid().max(dim=2)[0]
...
def main():
    ...
    parser.add_argument("--tag", type=str, default=None,
                        help="결과 출력/저장 시 식별용 라벨 (예: A_baseline_zero_shot).")
    ...
    for group in ("all", "spatial", "non_spatial"):
        ...
        t1 = r["top1"]
        tk = r[topk_key]
        t1_s = f"{t1*100:6.2f}%" if t1 == t1 else "   n/a"
